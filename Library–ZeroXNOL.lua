...
local Library do
    ...
    local IsMobileDevice = IsMobile and (ScreenSize.X < 768 or ScreenSize.Y < 768)
    ...
    ScaleFactor = IsMobileDevice and 0.7 or 1, -- 移动端缩放70%
    MinimumTouchSize = 44, -- 最小触摸区域（像素）
}
...
Library.OptimizeTouchArea = function(self, Element, MinSize)
    if not Library.MobileConfig.Enabled then return end
    
    MinSize = MinSize or Library.MobileConfig.MinimumTouchSize
    ...
    Library:Connect(RunService.RenderStepped, function()
        ...
        if size.X < MinSize or size.Y < MinSize then
            ...
            if not Element.Instance:FindFirstChild("TouchPadding") then
                local touchPadding = Instances:Create("UIPadding", {
                    Parent = Element.Instance,
                    Name = "TouchPadding",
                    PaddingTop = UDimNew(0, padding),
                    PaddingBottom = UDimNew(0, padding),
                    PaddingLeft = UDimNew(0, padding),
                    PaddingRight = UDimNew(0, padding)
                })
            end
        end
    end)
    ...
    local DefaultSize = UDim2New(0, 751, 0, 539)
    if Library.MobileConfig.Enabled then
     
        local screenWidth = Camera.ViewportSize.X
        local screenHeight = Camera.ViewportSize.Y
        DefaultSize = UDim2New(0, math.min(screenWidth * 0.9, 500), 0, math.min(screenHeight * 0.8, 400))
    end

    local Window = { 
        Logo = Data.Logo or Data.logo or "",
        FadeTime = Data.FadeTime or Data.fadetime or 0.4,
        Size = Data.Size or Data.size or DefaultSize, 
        Pages = { },
        Items = { },
        IsOpen = false,
    }
    ...
    Position = UDim2New(0, 12, 0, 12),
    BorderColor3 = FromRGB(42, 49, 45),
    Size = Library.MobileConfig.Enabled and UDim2New(0, 150, 1, -24) or UDim2New(0, 200, 1, -24),
    BorderSizePixel = 2,
    BackgroundColor3 = FromRGB(20, 24, 21)
})
    ...
    Size = Library.MobileConfig.Enabled and UDim2New(1, -12, 0, 25) or UDim2New(0, 0, 0, 20),
    BorderSizePixel = 2,
    AutomaticSize = Library.MobileConfig.Enabled and Enum.AutomaticSize.None or Enum.AutomaticSize.X,
    BackgroundColor3 = FromRGB(14, 17, 15)
})
...
Items["Pages"] = Instances:Create("ScrollingFrame", {
    ...
    BorderSizePixel = 0,
    ScrollBarThickness = Library.MobileConfig.Enabled and 4 or 2,
    ScrollBarImageColor3 = FromRGB(202, 243, 255),
    CanvasSize = UDim2New(0, 0, 0, 0),
    AutomaticCanvasSize = Enum.AutomaticSize.Y,
    BackgroundColor3 = FromRGB(255, 255, 255)
...
if Library.MobileConfig.Enabled then
    ...
    Name = "\0",
    BackgroundTransparency = 1,
    Position = Library.MobileConfig.Enabled and UDim2New(0, 166, 0, 12) or UDim2New(0, 226, 0, 12),
    BorderColor3 = FromRGB(0, 0, 0),
    Size = Library.MobileConfig.Enabled and UDim2New(1, -178, 1, -24) or UDim2New(1, -238, 1, -24),
    BorderSizePixel = 0,
    BackgroundColor3 = FromRGB(255, 255, 255)
})
